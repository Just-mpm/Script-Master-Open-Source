...
### Vídeo (Remotion)
Renderização client-side via WebCodecs com fallback de codec (H.264+AAC → H.264 → VP8+Opus+WebM). Legendas: pipeline 3 fontes (segment-timing > whisper-aligned > proportional). Speed Paint: edge detection + BFS + renderização Remotion com fases sketch/reveal. Timings centralizados em `speedPaintTimings.ts` (`DEFAULT_SPEED_PAINT_HOLD_SECONDS=3s`, `DURATION_BASED_SKETCH_RATIO=0.8`). Web Worker inline para >5 cenas. Cache LRU (SHA-256, 20 entradas, chave inclui `mode + preset` no v0.131.0). Export quality: 720p–4K.

**Speed Paint — dois modos de renderização (v0.131.0):**
- **`mask` (default, retrocompatível):** pipeline atual preservado — edge detection + BFS + tracing (Worker inline + fallback `processOnMainThread`). `StrokeAnimation` com `strokes[]` raster. Sem mudança de comportamento para projetos existentes.
- **`vetorial` (novo):** `vectorizeImage()` (Fase 1.2) na main thread converte `ImageData` em paths SVG via `imagetracerjs@1.2.6`. `WhiteboardScene` (em `src/features/video-render/components/`) renderiza paths crescendo sequencialmente com `strokeDashoffset` + caneta SVG inline (`Pencil` componente) seguindo a ponta do traço via `getPointAtLength()`. `VetorialAnimation` com `paths[]` pré-calculados (`length` calculado via `@remotion/paths.getLength()` na vetorização, nunca em runtime).
...
- **Discriminação polimórfica:** `PaintingJob.animation: StrokeAnimation | VetorialAnimation`; `SpeedPaintPlayer`, `SpeedPaintExportPanel` e `useSpeedPaintExporter` aceitam a união, discriminando via type guard real (`'paths' in animation` — narrowing type-safe, sem `as` bypass). `SpeedPaintPlayer` tem sub-componentes `MaskPlayer` / `VetorialPlayer` com SRP; `imageSource` tornado opcional.
- **`WhiteboardComposition`** em `src/features/speed-paint/components/`: wrapper Remotion do `WhiteboardScene`, análogo ao `SpeedPaintComposition`. Composição lazy `createExportableWhiteboardComposition()` no `speedPaintRenderController.tsx` com `compositionId` único (`'script-master-speed-paint-vetorial-export'`).
- **Seletor de modo na UI** (`SpeedPaintPage.tsx`): `ToggleButtonGroup` com ícones distintos (`FormatPaintOutlined` para Clássico, `GestureOutlined` para Desenho), glow no estado ativo, `aria-label` + `aria-describedby` + `Tooltip describeChild` para acessibilidade WCAG 2.1 AA. Troca dispara `trackAnalyticsEvent('speed_paint_mode_changed', { mode })`.
- **Persistência dual storage:** campo `speedPaintRenderMode` em `UserSetting` e `StudioUserSettings`; hook `useSyncSpeedPaintRenderMode` (debounce 2s) sincroniza com Firestore/IndexedDB. Mesmo padrão de `useAutoSaveStudioSettings`.
- **i18n** (3 locales): `modeLabel`, `modeClassic`, `modeVetorial`, `modeDescription` no namespace `speedPaint`.
- **Performance:** `MAX_PATHS_PER_SCENE = 500` com truncamento + `log.warn` (em `vectorizer.ts`); `PATHOMIT_BY_PRESET` heurístico por preset (presets "ricos" como `'detailed'`/`'artistic4'` têm `pathomit` mais alto); latência < 500ms para imagens 1920×1080. 33 novos testes em 4 arquivos: 22 unitários (vectorizer), 9 integração (pipeline), 2 e2e (10 imagens diversas), 4 componente (player). 2268/2268 testes passando.
- **Limitação documentada:** batch vetorial não suportado nesta versão (`runBatchRender()` só mask); snapshot/render de componentes Remotion fora do escopo (pioneirismo).
- **Dependências adicionadas:** `imagetracerjs@1.2.6` e `@remotion/paths@4.0.448`.
