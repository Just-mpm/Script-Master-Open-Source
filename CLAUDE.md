...
- **Tipos `vetorial.ts`**: `SpeedPaintRenderMode` (discriminated union `'mask' | 'vetorial'`), `VetorialPreset` (20 valores em 7 grupos), `VetorialPath`, `VetorialAnimation`, `VetorialPathSortOrder` (4 valores), `VetorialEasingType` (3 valores), `EdgePresetName` (6 valores). Re-exportados de `types.ts` para evitar import circular.
...
- **i18n** (3 locales): `modeLabel`, `modeClassic`, `modeVetorial`, `modeDescription`, `presetGroups` (7 grupos), `presets` (20 labels), `sortOrder*`, `easing*` no namespace `speedPaint`.
- **Performance:** `MAX_PATHS_PER_SCENE = 500` com truncamento + `log.warn` (em `vectorizer.ts`); `PATHOMIT_BY_PRESET` heurístico; `BACKGROUND_FILTER_WARN_RATIO = 0.5`. Pipeline edge+bezier: latência < 1000ms para imagens 1920×1080 (Canny + tracing + fitting). Fallback seguro para imagens sem bordas detectadas.
- **Batch vetorial suportado** (v0.132.0): `BatchOrchestrator` lê `renderMode`/`vetorialPreset` da store via `getState()` com race protection (`processingIdRef`). `speedPaintRenderController.tsx` sem restrição de batch vetorial.
- **Dependências adicionadas:** `imagetracerjs@1.2.6` e `@remotion/paths@4.0.448` (v0.131.0). Sem novas dependências na v0.132.0 (edge detection, contour tracing e bezier fitting são implementações nativas).
