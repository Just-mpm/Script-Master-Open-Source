...
- **`vetorial` (novo):** dois pipelines de vetorização coexistem (selecionados por preset):
  - **Pipeline legado (imagetracerjs):** `vectorizeImageLegacy()` converte `ImageData` em paths SVG via `imagetracerjs@1.2.6` — usado por presets clássicos, posterizados, artísticos e detalhados
  - **Pipeline edge+bezier (v0.132.0):** `vectorizeImageEdgeBezier()` — Canny edge detection (`edgeDetection.ts`) → Moore-Neighbor contour tracing (`contourTracing.ts`) → cubic Bézier fitting (`bezierFitting.ts`) — usado por presets `edge-*` (borda fina, média, grossa). Produz paths mais suaves com menos pontos. `sortPaths()` com 4 estratégias (`top-down`, `center-out`, `big-first`, `natural`) + `filterPathsByBackgroundContrast()` para remoção de paths de fundo
- **`WhiteboardScene`** (em `src/features/video-render/components/`): renderiza paths SVG crescendo sequencialmente com `strokeDashoffset` + caneta SVG inline (`Pencil` componente). Easing function plugável (`linear`, `smooth`, `bounce`). Motion blur na caneta (baseado em velocidade do traço). Tremor orgânico (`Math.sin`) para efeito de mão humana. `safeGetPointAtLength()` com fallback seguro. `TRANSITION_FRACTION` (0.05) controla transição entre strokes na animação da caneta.
- **Tipos `vetorial.ts`**: `SpeedPaintRenderMode` (discriminated union `'mask' | 'vetorial'`), `VetorialPreset` (presets remanescentes em 7 grupos após remoção dos legados), `VetorialPath`, `VetorialAnimation`, `VetorialPathSortOrder` (4 valores), `VetorialEasingType` (3 valores), `EdgePresetName` (6 valores). Re-exportados de `types.ts` para evitar import circular.
- **Seletor de modo na UI** (`SpeedPaintPage.tsx`): `ToggleButtonGroup` com ícones distintos (`FormatPaintOutlined` para Clássico, `GestureOutlined` para Desenho), glow no estado ativo. Abaixo do toggle: `<Select>` de preset (20 opções em 7 grupos com ícones), `<Select>` de sort order (4 opções com tooltips), `<Select>` de easing (3 opções). Guardas de runtime para validar valores de selects.
...
- **i18n** (3 locales): `modeLabel`, `modeClassic`, `modeVetorial`, `modeDescription`, `presetGroups` (7 grupos), `presets` (20 labels), `sortOrder*`, `easing*`, `sceneRenderMode` no namespace `speedPaint`.
- **Performance:** Pipeline edge+bezier: latência < 1000ms para imagens 1920×1080 (Canny + tracing + fitting). Fallback seguro para imagens sem bordas detectadas. `filterContoursByCompactness()` classifica contornos por área/perímetro antes da vetorização. `BACKGROUND_FILTER_WARN_RATIO = 0.5`. Sem `MAX_PATHS_PER_SCENE` nesta versão — a compactação é feita por filtro geométrico.
- **Batch vetorial suportado**: `BatchOrchestrator` lê `renderMode`/`vetorialPreset` da store via `getState()` com race protection (`processingIdRef`). `speedPaintRenderController.tsx` sem restrição de batch vetorial. Pipeline vetorial pode rodar no Web Worker (`vetorialWorker.ts`) via hook `useEdgeWorker` em `imageProcessing.ts`.
- **Web Worker para pipeline vetorial** (v0.133.0): `vetorialWorker.ts` (139 linhas) processa edge+bezier off the main thread. Elimina bloqueio da UI em roteiros com muitas cenas. Fallback para thread principal quando worker não é suportado. Integrado via `useEdgeWorker` e `processVetorialInWorker`.
...
- **Dependências adicionadas:** `imagetracerjs@1.2.6` e `@remotion/paths@4.0.448` (v0.131.0). Sem novas dependências na v0.132.0 ou v0.133.0 — edge detection, contour tracing, bezier fitting e vetorialWorker são implementações nativas do projeto.
...
| `0.131.0` | **Modo "Desenho" (vetorial) no Speed Paint:** novo modo de renderização que substitui a revelação por máscara (raspadinha) por animação whiteboard — paths SVG crescendo sequencialmente com `strokeDashoffset` e caneta SVG seguindo a ponta do traço via `getPointAtLength()`. Vetorização via `imagetracerjs@1.2.6` + `getLength()` do `@remotion/paths@4.0.448`. Retrocompatível: `renderMode: 'mask'` default. `WhiteboardScene` determinístico, `WhiteboardComposition` lazy, `SpeedPaintPlayer` estendido para aceitar `StrokeAnimation \| VetorialAnimation` via type guard real. Seletor de modo na UI com ícones distintos e analytics. Persistência em `UserSettings` (dual storage). i18n (3 locales, 4 chaves). `MAX_PATHS_PER_SCENE=500` + `PATHOMIT_BY_PRESET`. 33 novos testes em 4 arquivos. 2268/2268 passando. Limitação: batch vetorial não suportado |
| `0.133.0` | **Web Worker para pipeline vetorial e SceneRenderModePanel:** novo worker dedicado (`vetorialWorker.ts`) para processar Canny edge detection, contour tracing e Bézier fitting off the main thread — elimina bloqueio da UI em roteiros com muitas cenas. Novo componente `SceneRenderModePanel` para seleção de modo de renderização por cena no editor de vídeo. Utilitários de geometria adicionados (`polygonArea`, `polygonPerimeter`, `filterContoursByCompactness`). Namespace i18n `sceneRenderMode` nos 3 locales. Presets legados removidos (`posterized1/2/3`, `curvy`), padrão alterado para `edge-default`. Removida `isValidProjectVideo` de `firestore.rules`. |
| `0.132.0` | **Pipeline edge+bezier para modo vetorial no Speed Paint:** novo motor de vetorização com Canny edge detection (`edgeDetection.ts`) → Moore-Neighbor contour tracing (`contourTracing.ts`) → cubic Bézier fitting (`bezierFitting.ts`) — paths mais suaves e com menos pontos. Coexiste com pipeline legado (imagetracerjs). `sortPaths()` com 4 estratégias, `filterPathsByBackgroundContrast()`, `safeGetPointAtLength()`, easing plugável (`linear/smooth/bounce`), motion blur + tremor orgânico na caneta. 20 presets em 7 grupos na UI. Sort order e easing persistidos em dual storage. Batch vetorial suportado. `VetorialPathSortOrder` (4 valores), `VetorialEasingType` (3 valores), `EdgePresetName` (6 valores). 14+ novos arquivos de teste. Limitação: easing select não exposto na UI (default `smooth`) |
| `0.131.0` | **Modo "Desenho" (vetorial) no Speed Paint:** novo modo de renderização que substitui a revelação por máscara (raspadinha) por animação whiteboard — paths SVG crescendo sequencialmente com `strokeDashoffset` e caneta SVG seguindo a ponta do traço via `getPointAtLength()`. Vetorização via `imagetracerjs@1.2.6` + `getLength()` do `@remotion/paths@4.0.448`. Retrocompatível: `renderMode: 'mask'` default. `WhiteboardScene` determinístico, `WhiteboardComposition` lazy, `SpeedPaintPlayer` estendido para aceitar `StrokeAnimation \| VetorialAnimation` via type guard real. Seletor de modo na UI com ícones distintos e analytics. Persistência em `UserSettings` (dual storage). i18n (3 locales, 4 chaves). `MAX_PATHS_PER_SCENE=500` + `PATHOMIT_BY_PRESET`. 33 novos testes em 4 arquivos. 2268/2268 passando. Limitação: batch vetorial não suportado |
| `0.130.3` | **Correções de upload e drop visual no SpeedPaint:** `dropAnimation={null}` no `<DragOverlay>` do `@dnd-kit/react` elimina flash visual ao soltar imagem no `QueueStaging.tsx`; conflito entre `<Button component="label">` e `useDropzone` resolvido em `ImageUpload.tsx` — extraído `open()` com `noClick: true` + `noKeyboard: true`. 5 novos testes de acessibilidade. 4 docs de auditoria adicionados |
