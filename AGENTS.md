...
### Autenticação
`AuthContext` + `useAuth()`: Google popup, email/senha com verificação (polling 5s), reset de senha, exclusão LGPD. Onboarding Wizard (`/onboarding`): 4 passos (Welcome → Profile → Goals → Completion), 6 roles, 8 goals — persistido em localStorage + `user_settings` no Firestore. Pós-login: sem onboarding → `/onboarding`, completo → `/app/estudio`. Login/logout/delete fazem full reload (COEP conflict).
