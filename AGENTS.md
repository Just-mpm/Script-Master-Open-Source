...
| | |
|---|---|
| **Arquivos** | `cloud-run/Dockerfile`, `cloud-run/package.json`, `cloud-run/tsconfig.json`, `cloud-run/.dockerignore`, `cloud-run/scripts/deploy.ps1`, `cloud-run/src/index.ts`, `cloud-run/src/renderer.ts`, `cloud-run/src/firebase.ts`, `cloud-run/src/types.ts`, `cloud-run/src/logger.ts`, `cloud-run/remotion/index.tsx`, `cloud-run/src/speed-paint/` (5 arquivos: `index.ts`, `types.ts`, `cache.ts`, `imageProcessing.ts`, `speedPaintStrokes.ts`) |
...
| **Dependências** | `@remotion/bundler`, `@remotion/renderer`, `express`, `firebase-admin`, `sharp` ^0.33.5 (processamento de imagens server-side para speed paint) |
...
| Versão | Resumo |
|--------|--------|
| 0.50.1 | **Cancelamento cooperativo no Cloud Run + SceneItem memoizado + timeout no stroke worker + auditoria** — `generateStrokesForScenes()` agora aceita `cancelSignal` e `targetWidth` no renderer Cloud Run; novo componente `SceneItem` memoizado no `VideoComposition` para performance; `TIMEOUT_MS (60s)` adicionado ao stroke worker para prevenir travamentos; `WaveformOverlay` migrado para `useMemo`; novo relatório de auditoria em `docs/audits/audit-target-closed-speedpaint.md` |
| 0.50.0 | **Módulo speed-paint no Cloud Run + sharp + refatoração de env vars** — novo módulo `cloud-run/src/speed-paint/` com 5 arquivos para geração server-side de strokes de speed paint usando sharp; dependência `sharp` adicionada ao Cloud Run; `generateStrokesForScenes()` integrado ao renderer para animar cenas do pipeline server-side; body limit Express aumentado para 50mb; `start-video-job.ts` agora verifica `CLOUD_RUN_URL` em vez de `CLOUD_RUN_VIDEO_ENABLED`; `useVideoExporter.tsx` limpo de `useCloudRun` e `speedPaintWarnings`; storage rules para cache de speed paint |
