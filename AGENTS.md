...
bun run deploy:storage:cors # atualiza CORS do Storage bucket via gcloud
...
- **Firebase Cloud Functions v2** — backend serverless com Genkit (26 flows de IA: audio, startAudioJob, processAudioJob, cancelAudioJob, images, startImageJob, processImageJob, cancelImageJob, scenePrompts, startScenePromptJob, processScenePromptJob, assistant, inline-assistant, chunking, feedback, ping, audio-preflight, cancel-ai-request, cancelJob, credit-snapshot, startPipeline, cancelPipeline, onSubJobCompleted, processVideoJob, onVideoJobTerminal, deleteJob) + Stripe em `functions/`
...
- **Cloud Run** — renderização de vídeo Remotion server-side via Docker + `@remotion/renderer` (timeout 60min, 4 vCPUs + 16 GiB, min 1 instância)
...
| **Stroke worker** | `strokeWorker.ts` — `createStrokeWorker()`, `terminateStrokeWorker()`, `processSceneInWorker()`, `supportsStrokeWorker()`. Timeout de 60s (`TIMEOUT_MS`) para prevenir travamentos do worker |
| **Animação server-side** | `SpeedPaintScene` aceita `strokeAnimationUrl` para carregar animações do Storage (geradas pelo Cloud Run) — `fetchStrokeAnimation()` busca e hidrata strokes remotos. Campos `strokeAnimationUrl`, `speedPaintStatus`, `speedPaintError` nas interfaces de cena (`SceneWithStrokeUrl`) permitem diagnóstico e fallback transparente |
...
| **Pipeline Server-Side** | `startPipeline` (Cloud Function callable) orquestra o pipeline completo (áudio → scene prompts → imagens → vídeo) no backend. `onSubJobCompleted` (onDocumentWritten) avança automaticamente entre etapas. Opções `animateScenes` e `includeSubtitles` controlam animação de cenas (speed paint) e legendas no vídeo final. Steps de áudio, cenas e imagens usam Cloud Tasks; o step de vídeo usa `processVideoJob` via Cloud Tasks queue que dispara um Cloud Run Job. `buildPipelineCaptions()` (via `video-captions.ts`) constrói legendas do pipeline. `getResolutionFromRatioAndQuality()` calcula resolução baseada em proporção + qualidade. `usePipelineOrchestrator` no frontend chama `startPipeline` e monitora o progresso via Firestore. Cancelamento em cascata via `cancelPipeline` com `cancelPipelineNow()` propagando para todos os sub-jobs. `onVideoJobTerminal` (onDocumentWritten) monitora jobs de vídeo em status terminal. `deleteJob` (callable) permite deleção manual de jobs |
| **Feature flags** | `VITE_CLOUD_RUN_VIDEO_ENABLED` (renderização Cloud Run) — default `false`, rollback instantâneo. Backend `start-video-job.ts` verifica `CLOUD_RUN_URL` diretamente como gate de disponibilidade |
| **Cleanup** | Cloud Function agendada `cleanupOldJobs` (onSchedule diário 03:00 BRT) — remove completed >30d, failed >7d, cancelled >3d; `markStuckJobsAsFailed()` marca jobs presos em running/queued >24h como failed; `reconcileQueuedCancelledJobs()` reconcilia jobs que ficaram em `queued` após cancelamento de pipeline |
...
| **useJobsStore** | Zustand — agrega 4 onSnapshot listeners em `AnyJob[]` unificado. Ações: `syncAuth(uid)`, `reset()`, `removeJob(jobId)`. Selectors: `useActiveJobs`, `useActiveJobCount`, `useJobsByType`, `useJobById`, `useFinishedJobs`. Pipeline jobs mapeados via `mapPipelineJob()` |
...
| | |
|---|---|
| **Arquivos** | `cloud-run/Dockerfile`, `cloud-run/package.json`, `cloud-run/tsconfig.json`, `cloud-run/.dockerignore`, `cloud-run/scripts/deploy.ps1`, `cloud-run/src/index.ts`, `cloud-run/src/renderer.ts`, `cloud-run/src/firebase.ts`, `cloud-run/src/types.ts`, `cloud-run/src/logger.ts`, `cloud-run/src/storageDownload.ts`, `cloud-run/remotion/index.tsx`, `cloud-run/src/speed-paint/` (5 arquivos: `index.ts`, `types.ts`, `cache.ts`, `imageProcessing.ts`, `speedPaintStrokes.ts`), `functions/src/lib/cloud-run-jobs.ts` (cliente Cloud Run Jobs API) |
...
| **API** | Express (service auxiliar): `POST /render` (responde com resultado síncrono: `completed`/`error`, renderiza inline) + `GET /health`. **Cloud Run Job** (worker oficial): entry point `run-render-job.js` acionado via Cloud Tasks → `processVideoJob` → `loadRenderRequestFromJob()` + `renderVideoJob()`. Ambos com auth via IAM Invoker (mesmo projeto GCP, rede VPC) |
| **Renderer** | `@remotion/renderer` com `selectComposition` + `renderMedia` + `makeCancelSignal`. Progresso no Firestore throttled (1 update/s). Cancelamento via polling `cancelRequested` a cada 5s. Upload do vídeo final para Firebase Storage com token de download persistente (`createDownloadToken`, `buildStorageDownloadUrl` — funções em `storageDownload.ts`). **Speed paint server-side**: Stage 0 no render — `generateStrokesForScenes({ cancelSignal, targetWidth })` (via módulo `speed-paint/` com sharp) gera strokes para cenas com `animateScenes: true` antes do bundling; `cancelSignal` integra com `makeCancelSignal` para cancelamento cooperativo; falha degrada gracefulmente. Rastreamento granular de status por cena (`SpeedPaintSceneStatus`, `SpeedPaintScenePayload`) com sumário (`SpeedPaintRenderSummary`) persistido no job doc do Firestore. **Cloud Run Job mode**: `loadRenderRequestFromJob()` carrega `RenderRequest` do Firestore; `renderVideoJob()` orquestra renderização completa com upload e atualização de progresso |
| **Config (service)** | 4 vCPUs + 16 GiB, timeout 30min, concorrência 1, escala a zero (min instances = 1, max = 5) |
| **Config (job)** | 4 vCPUs + 16 GiB, timeout 60min, 1 task, 0 retries, paralelismo 1 |
| **Deploy** | Script PowerShell `deploy.ps1` com pipeline de 8 etapas: verificação (gcloud/Docker) → typecheck → build TS → Docker build/push → Cloud Run service deploy (min-instances 1, no-cpu-throttling) → health check com retry → Cloud Run Job deploy (create/update, 60min timeout, 0 retries). Comando raiz: `bun run deploy:cloudrun` |
| **Dependências** | `@remotion/bundler`, `@remotion/renderer`, `express`, `firebase-admin`, `sharp` ^0.33.5 (processamento de imagens server-side para speed paint). **Cloud Run Jobs client**: `functions/src/lib/cloud-run-jobs.ts` (API REST nativa, sem dependência adicional) |
...
| Versão | Resumo |
|--------|--------|
| 0.52.1 | **Copy overhaul das páginas públicas (tom casual-confiante) + grupo "Recursos" no footer + refinamentos responsivos de layout** — revisão completa do copy em pt-BR, en e es (landing, features, pricing FAQ, uso, métricas, depoimentos); novo grupo `resourcesGroup` no `PublicFooter` com links Roadmap/Changelog/Comunidade e chaves i18n correspondentes nos 3 locales; `fontSize` e `maxWidth` responsivos no `CTASection`; `fontSize` explícito no `FeatureCard` e `FeatureShowcase`; `pt`/`mt` responsivos na `LandingPage` e `FuncionalidadesPage` |
| 0.52.0 | **Cloud Run Jobs + módulo de job cancel helpers + onVideoJobTerminal + deleteJob + strokeAnimationUrl + refinamentos de UI** — novo cliente Cloud Run Jobs API em `functions/src/lib/cloud-run-jobs.ts`; Cloud Run Job `remotion-renderer-job` como worker oficial de render com 60min timeout; novas Cloud Functions `onVideoJobTerminal` (Firestore trigger) e `deleteJob` (callable); módulo `job-cancel-helpers.ts` com `cancelQueuedSubJobNow()` e `SubJobType`; `loadRenderRequestFromJob()` e `renderVideoJob()` no renderer; campo `strokeAnimationUrl` para carregamento server-side de strokes; `normalizeProgressLabel()` na UI de jobs; deploy script atualizado para deploy de service + job |
| 0.51.0 | **Extração do módulo storageDownload + tipos de status de speed paint + appendStrokes + cache assíncrono + testes unitários** — novo módulo `cloud-run/src/storageDownload.ts` com `createDownloadToken()`, `buildStorageDownloadUrl()`, `getPrimaryDownloadToken()` extraídos do renderer e cache para responsabilidade única; interfaces `SpeedPaintScenePayload`, `SpeedPaintRenderSummary`, `SpeedPaintSceneStatus`, `ProcessSceneResult`, `CachedAnimationRecord` adicionadas para rastreamento granular de status de speed paint por cena; função `appendStrokes()` no imageProcessing; `saveToCache()` e `readFromCache()` refatorados para assíncronos com suporte a download tokens; campos `speedPaintStatus`/`speedPaintError` propagados do backend Remotion até o job doc no Firestore; testes unitários para `imageProcessing` e `storageDownload` |
| 0.50.3 | **Refatoração de metadados de vídeo no Cloud Run + correção de dependência faltante no WaveformOverlay** — `getVideoMetadata` substituída por `getVideoCompositionMetadata()` + `loadStrokeAnimations()` com interfaces `SceneWithStrokeUrl` e `VideoMetadataResult` no módulo Remotion do Cloud Run, separando extração de metadados do carregamento de animações; dependência `fps` adicionada ao `useMemo` do `WaveformOverlay` para correção de recálculo de duração de cena |
| 0.50.2 | **Aumento de memória Cloud Run para 16Gi + guard de continueRender + sharp.cache(false) + refatoração de cancel signal** — memória do serviço Cloud Run aumentada de 8Gi para 16Gi para suportar múltiplas cenas em altas resoluções; flag `hasCalledContinueRender` adicionada ao `SpeedPaintScene` para eliminar double continueRender; `sharp.cache(false)` no módulo speed-paint server-side para prevenir OOM; lógica de cancelamento extraída do loop do `generateStrokesForScenes()` para evitar stale closure |
